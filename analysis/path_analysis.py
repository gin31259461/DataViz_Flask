...
class DataAnalysis:
    ...
    def start_analysis(self):
        self.fetch_data()
        self.handle_numerical_column()
        self.parse_datetime_column()
        self.handle_datetime_column()
        self.handle_concept_hierarchy()
        self.handle_fitting_data()
        self.encode_category_column()
        self.fit_decision_tree_model()
        self.store_data_information()
        self.fetch_decision_tree_model_path()

    def fetch_data(self):
        query = text(f"SELECT * FROM [RawDB].[dbo].[D{self.OID}]")
        data = db.execute(query)

        query = text("SELECT * FROM [DV].[dbo].[Object] where OID = :OID")
        object_table = db.execute(query, OID=self.OID).fetchall()

        self.df = pd.DataFrame(data.fetchall())
        self.original_df = self.df.copy()

        data_name = pd.DataFrame(object_table)["CName"][0]

        self.column_names = self.df.columns.tolist()
        print(f"fetch data successfully!\ndata name: {data_name}\ncolumn names: {self.column_names}")

    def handle_numerical_column(self):
        self.quantile_mapping = {}

        numerical_column_tuple: Tuple[str, int, int] = []
        for col in self.column_names:
            column_ratio = len(self.df[col].unique()) / self.df[col].count()
            is_categorical_column = (
                self.df[col].dtype == "object" or self.df[col].dtype == "category" or column_ratio < 0.01
            )
            is_numerical_column = self.df[col].dtype == "int64" and not is_categorical_column
            if is_numerical_column:
                numerical_column_tuple.append([col, self.df[col].min(), self.df[col].max()])
        ...
        for tuple in numerical_column_tuple:
            ...
            quantile = pd.qcut(self.df[col], q=[0, 0.1575, 0.8425, 1])
            self.df[col] = pd.qcut(self.df[col], q=[0, 0.1575, 0.8425, 1], labels=quantile_labels)
            self.quantile_mapping[col] = pd.Series(
                data=quantile.unique().tolist(), index=self.df[col].unique().tolist()
            )

        self.numerical_column_tuple = numerical_column_tuple
    ...
    def parse_datetime_column(self):
        ...
        for col in self.column_names:
            is_datetime = self.df[col].dtype == "datetime64[ns]"
            if is_datetime:
            ...
            for test_format in datetime_format_list:
                is_numeric = self.df[col].dtype == "int64"
                is_datetime = is_numeric and (
                    True
                    not in pd.to_datetime(arg=self.df[col].astype("str"), format=test_format, errors="coerce")
                    .isna()
                    .value_counts()
                    .index.to_list()
                )
                if is_datetime:
                    parsed_datetime = pd.to_datetime(arg=self.df[col], format=test_format, errors="coerce")
                    ...
                    break
            self.datetime_column_tuple = datetime_column_tuple

    def count_feature_gain(self, feature_name: str):
        part_df = pd.DataFrame(columns=[feature_name, self.target])
        part_df[feature_name] = self.df[feature_name].astype("str")
        part_df[self.target] = self.df[self.target]
    ...
    def fill_label(self, freq: str, datetime_manifest: Type[pd.DatetimeIndex]):
        ...
        match freq:
            ...
            #             lower_bound = datetime_manifest[i - 1].month
            #             higher_bound = datetime_manifest[i].month
            #             year = datetime_manifest[i - 1].year
            ...
            #                 case [10, 1]:
            #                     labels.append(f"{year}-Q4")
            #                 case _:
            #                     label_mapping = pd.Series(data=["Q1", "Q2", "Q3", "Q4"], index=[1, 4, 7, 10])
            #                     labels.append(str(year) + "-" + label_mapping[lower_bound])
            #     case "month":
            #         labels = [f"{datetime.year}-{datetime.month}" for datetime in list(datetime_manifest)]
            #         if len(labels) >= 0:
            #             labels.pop()
            #     case _:
            #         pass
    ...
    def count_target_entropy(self):
        total_quantity = len(self.df[self.target])
        class_value_quantity = len(self.df[self.target].unique())
        H_s = [
            -(target_value / total_quantity) * log(target_value / total_quantity, class_value_quantity)
            for target_value in self.df[self.target].value_counts().values.tolist()
        ]
        entropy = sum(H_s)
        return entropy

    def handle_datetime_column(self):
        ...
        for tuple in self.datetime_column_tuple:
            ...
            for datetime_freq in datetime_freq_manifest:
                ...
                discrete_datetime_series = pd.Series(
                    pd.cut(self.df[col], bins=datetime_manifest, labels=labels, include_lowest=True, ordered=False)
                )
            ...
            print(f"Origin 年月 entropy: {self.count_feature_gain('年月')}")
            ...
            datetime_manifest = datetime_manifest.union([tuple[2]])
            labels = self.fill_label(best_freq, datetime_manifest)

            quantile_interval = pd.cut(self.df[col], bins=datetime_manifest, include_lowest=True)
            self.quantile_mapping[col] = pd.Series(data=quantile_interval.tolist(), index=self.df[col].tolist())

            self.df[best_freq] = pd.Series(
                pd.cut(self.df[col], bins=datetime_manifest, labels=labels, include_lowest=True, ordered=False)
            )
            self.skip_features.append(col)
            self.datetime_columns = datetime_columns

    def handle_concept_hierarchy(self):
        print(f"Target origin entropy: {self.count_target_entropy()}")
        ...
        for col in self.concept_hierarchy:
            ...
            for concept in hierarchy:
                self.df[col] = self.df[col].replace(to_replace=hierarchy[concept], value=concept)
            if "order" in self.concept_hierarchy[col]:
                order = self.concept_hierarchy[col]["order"]
                self.df[col] = pd.Categorical(values=self.df[col], categories=order, ordered=True)
            print(f"After generalize {col} entropy:", self.count_feature_gain(col))

    def handle_fitting_data(self):
        self.df = self.df.drop_duplicates(keep="first")

        X: pd.DataFrame
        try:
            X = self.df.drop([self.target] + self.skip_features, axis=1)
        except KeyError:
            print("Column of target or skip features are not exist in data frame")

        self.X = X
        self.feature_names = X.columns.tolist()
        self.y = self.df[self.target].astype("string")
    ...
    def DecisionTreePathResolver(self, graph: DecisionTreeGraph, root_id: int = 0):
    ...
    def DecisionTreePathAnalyzer(self, paths: list[DecisionTreePath]):
        target_values = self.class_names
        ...
        for path in paths:
            ...
            for feature_name in feature_names:
                path_analysis_result_part[feature_name] = self.data_information["feature_values"][feature_name][
                    "value"
                ].copy()
            ...
            for node_id in path.path:
                ...
                feature_type = self.data_information["feature_values"][labels[0]]["type"]
                split_situation = [split_symbol, feature_type]
                mapping: pd.Series = self.data_information["feature_values"][feature_name]["mapping"]
    ...
    def fetch_decision_tree_model_path(self):
        ...
        paths = self.DecisionTreePathResolver(decision_tree_graph, 0)
        print("Path counts = {}".format(len(paths)))
        self.result = self.DecisionTreePathAnalyzer(paths)

    def store_data_information(self):
        data_information: dict[str, str or list or dict] = {}
        data_information["target_name"] = self.target
        data_information["target_values"] = self.class_names
        data_information["feature_names"] = self.feature_names

        # Numeric
        feature_series: dict[str, dict[str, str or list]] = {}

        for n in self.numerical_column_tuple:
            feature_series[n[0]] = {"type": "numeric", "value": [n[1], n[2]]}

        # Datetime
        for part_df in self.datetime_column_tuple:
            format = "%Y-%m-%d %X"
        ...
        for f in unstored_features:
            split_value = self.df[f].unique().astype("str").tolist()
        ...
        data_information["feature_values"] = feature_series
        self.data_information = data_information

    def count_attribute_value_distribution(self, data: Type[pd.DataFrame], attr_name: str):
        value_counts: Type[pd.Series[int]] = data[attr_name].value_counts()
        value_sum = sum(value_counts)
        distribution = pd.Series([0 for i in range(len(value_counts.index))], index=value_counts.index)

        for index in value_counts.index:
            distribution[index] = value_counts[index] / value_sum

        print(distribution)

    def path_data_to_json(self):
        return json.dumps(self.result)


# count_attribute_value_distribution(original_df.filter(part_df.index, axis=0), "教育程度類別")
